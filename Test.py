...
class NeuralNetwork:
    ...
    def forward(self, X):
        ...
        for i in range(len(self.weights)-1):  # all hidden layers
            z = activations[-1] @ self.weights[i] + self.biases[i] # z = wx+b
            zs.append(z)
            a = self.act(z)
            activations.append(a)

        # Output layer = Softmax only (NO relu/sigmoid)
    ...
    def backward(self, activations, zs, y_true):
        ...
        # ★ MODIFIED: delta for final layer (Softmax + CE)
        ...
        grad_w = activations[0].T @ delta / m
        grad_b = np.sum(delta, axis=0, keepdims=True) / m
        grads_w.insert(0, grad_w)
        grads_b.insert(0, grad_b)
